...
- **`pyrad2.eap` is now a package**, not a flat module. Every historical name (`build_eap_identity`, `inject_eap_identity`, `apply_eap_md5_challenge`, `password_from_packet`, `EAP_MESSAGE_ATTR`, `STATE_ATTR`, `USER_NAME_ATTR`, `USER_PASSWORD_ATTR`) is re-exported from `pyrad2.eap`, so existing `from pyrad2 import eap` (or `from pyrad2.eap import ...`) call sites need no changes. The new surface is the `EapMethod` ABC plus a `register_method` / `get_method` registry — see [Making RADIUS Requests](client.md#eap-md5).
- **`Client.retries` and `Client.timeout` (and the equivalents on `ClientAsync`) are now property proxies** over the new `RetryPolicy`. Reads and writes behave the same as before; the new `retry_policy=` constructor kwarg is opt-in for backoff / jitter.
