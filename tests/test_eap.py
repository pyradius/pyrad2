...
class BuildEapIdentityTests(unittest.TestCase):
    def test_payload_layout_matches_rfc3748(self):
        ...
        self.assertEqual(code, EAPPacketType.RESPONSE)
        self.assertEqual(eap_type, EAPType.IDENTITY)
        # length field counts the EAP payload itself (5 header + password).
        self.assertEqual(length, len(password) + 5)
        self.assertEqual(payload[5:], password)
...
class BuildEapMd5ChallengeTests(unittest.TestCase):
    def test_payload_layout_matches_rfc3748(self):
        eap_id = 7
        password = b"hunter2"
        ...
        self.assertEqual(code, EAPPacketType.RESPONSE)
        self.assertEqual(returned_id, eap_id)
        self.assertEqual(eap_type, 4)  # EAP-Type for MD5-Challenge.
...
class PasswordFromPacketTests(unittest.TestCase):
...
class InjectEapIdentityTests(unittest.TestCase):
    def test_populates_eap_message_attribute(self):
        pkt = {eap.USER_PASSWORD_ATTR: [b"hunter2"]}
        eap.inject_eap_identity(pkt)

        self.assertIn(eap.EAP_MESSAGE_ATTR, pkt)
        eap_message = pkt[eap.EAP_MESSAGE_ATTR][0]
        # Response code, Identity type, password trailer.
        self.assertEqual(eap_message[0], EAPPacketType.RESPONSE)
        self.assertEqual(eap_message[4], EAPType.IDENTITY)
        self.assertEqual(eap_message[5:], b"hunter2")


class ApplyEapMd5ChallengeTests(unittest.TestCase):
    ...
    def test_replaces_eap_message_and_copies_state(self):
        ...
        self.assertEqual(response[0], EAPPacketType.RESPONSE)
        self.assertEqual(response[1], 7)
        self.assertEqual(response[4], 4)
        # State must round-trip unchanged.
        self.assertEqual(pkt[eap.STATE_ATTR], [b"opaque-state"])
